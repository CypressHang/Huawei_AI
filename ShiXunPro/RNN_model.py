...
class BiRNN(object):
    ...
    def train(self):
        ...
        for epoch in range(epochs):  # 样本集迭代次数
            ...
            for batch in range(n_batches_epoch):  # 一次batch_size，取多少次
                ...
                next_idx, self.audio_features, self.audio_features_len, self.sparse_labels, wav_files = RNN_utils.next_batch(
                    next_idx,
                    batch_size,
                    n_input,
                    n_context,
                ...
                if (batch + 1) % 70 == 0:
                    ...
                    dense_labels = RNN_utils.trans_tuple_to_texts_ch(self.sparse_labels, self.words)

                    print('错误率: ', train_err)
                    for orig, decoded_array in zip(dense_labels, dense_decoded):
                        # convert to strings
                        decoded_str = RNN_utils.trans_array_to_text_ch(decoded_array, self.words)
    ...
    def test_target_wav_file(self, wav_files, txt_labels):
        ...
        self.audio_features, self.audio_features_len, text_vector, text_vector_len = RNN_utils.get_audio_mfcc_features(
            None,
            wav_files,
            n_input,
            n_context,
            self.word_num_map,
            txt_labels)
        self.sparse_labels = RNN_utils.sparse_tuple_from(text_vector)
        d, train_ler = self.sess.run([self.decoded[0], self.label_err], feed_dict=self.get_feed_dict(dropout=1.0))
        dense_decoded = tf.sparse_tensor_to_dense(d, default_value=-1).eval(session=self.sess)
        decoded_str = RNN_utils.trans_array_to_text_ch(dense_decoded[0], self.words)
    ...
    def variable_on_device(self, name, shape, initializer):
        #原来是with tf.device('/gpu:0'):  但是用不了 所以改成cpu:0试试
